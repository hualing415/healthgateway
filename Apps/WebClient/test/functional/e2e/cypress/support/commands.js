...
Cypress.Commands.add("login", (username, password, authMethod = AuthMethod.BCSC) => 
{
    cy.visit('/login')
    if (authMethod == AuthMethod.BCSC)
    {
        ...
        cy.url().should('contains', 'https://idtest.gov.bc.ca/login/identify')
    ...
    else
    {
        ...
        cy.get('#KeyCloakBtn').should('be.visible').should('have.text', 'KeyCloak').click()
